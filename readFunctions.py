...
def getPlayerStatsbyID(idval):
    ...
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()
    cursor.execute("SELECT eloRating, rocksThrown,papersThrow, scissorsThrown, gamesPlayed, gamesWon, PLAYER_STATS.playerID, PLAYER.username FROM PLAYER_STATS INNER JOIN PLAYER ON PLAYER.playerID = PLAYER_STATS.playerID WHERE PLAYER_STATS.playerID = ?", (idval,))
    returnData = [dict(row) for row in cursor.fetchall()]
    conn.commit()
    conn.close()
    return returnData
